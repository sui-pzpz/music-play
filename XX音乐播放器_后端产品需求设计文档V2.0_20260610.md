...
### 1.1 整体架构
```
客户端(Web/PC) → CDN → Nginx → API Gateway → 微服务集群
                                                    ├── 用户服务
                                                    ├── 音乐服务
                                                    ├── 歌单服务
                                                    ├── 搜索服务
...
| 组件 | 技术 | 版本 | 说明 |
|------|------|------|------|
| API框架 | Spring Boot / NestJS | 3.x / 10.x | RESTful API |
| 数据库 | MySQL | 8.0 | 主从复制，读写分离 |
| 缓存 | Redis | 7.x | 集群模式，热点数据缓存 |
| 搜索引擎 | Elasticsearch | 8.x | 全文搜索+联想词 |
| 消息队列 | RabbitMQ / Kafka | 3.x / 3.x | 异步处理、播放上报 |
| 文件存储 | OSS + CDN | - | 音频、图片、歌词 |
| 日志 | ELK Stack | 8.x | 日志采集+分析 |
...
| 服务 | 职责 | 端口 |
|------|------|------|
| user-service | 用户注册/登录/信息/权限 | 8001 |
| music-service | 歌曲元数据/播放地址/歌词/下载 | 8002 |
| playlist-service | 歌单CRUD/收藏/排序 | 8003 |
| search-service | 搜索/联想词/热门搜索 | 8004 |
| recommend-service | 推荐算法/推荐列表 | 8005 |
| member-service | 会员/订单/支付/权益 | 8006 |
| file-service | 文件上传/下载/CDN管理 | 8007 |
| notify-service | 消息通知/验证码/系统消息 | 8008 |
