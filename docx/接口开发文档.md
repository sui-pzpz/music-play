...
| Redis | 3.0+ | 缓存 |
| JWT | - | 认证方式 |
| Lombok | - | 代码简化（实体类已通过 delombok 生成显式 getter/setter） |
| Spring Data JPA | - | ORM 框架，使用 Repository + Specification 动态查询 |
...
> **排序字段说明**：前端发送的排序字段为 snake_case 格式（如 `created_at`、`play_count`），后端 `AdminSongServiceImpl` 通过 `toCamelCase()` 方法自动转换为 JPA 所需的 camelCase 格式（如 `createdAt`、`playCount`）。默认排序字段为 `created_at`。
...
**文档版本**：V1.2
**更新日期**：2026-06-13
**参考文档**：《后端开发文档20260610.md（V1.2）》《命名规范20260609.md》

### V1.2 更新说明

| 变更项 | 说明 |
| ------ | ---- |
| 技术栈补充 | 添加 Spring Data JPA 说明，更新 Lombok 说明（delombok 生成显式代码） |
| 歌曲排序字段 | 新增排序字段 snake_case → camelCase 自动转换说明，默认排序改为 `created_at` |
| 参考文档版本 | 更新为后端开发文档 V1.2 |