...
public interface UserRepository {
    ...
    @Select("SELECT * FROM t_user WHERE uid = #{uid} AND deleted_at IS NULL")
    User findByUid(@Param("uid") String uid);

    @Select("SELECT * FROM t_user WHERE phone = #{phone} AND deleted_at IS NULL")
    User findByPhone(@Param("phone") String phone);

    @Insert("INSERT INTO t_user (uid, phone, password, nickname, created_at, updated_at) " +
            "VALUES (#{uid}, #{phone}, #{password}, #{nickname}, NOW(), NOW())")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(User user);

    @Update("UPDATE t_user SET nickname=#{nickname}, avatar=#{avatar}, gender=#{gender}, " +
            "birthday=#{birthday}, signature=#{signature}, updated_at=NOW() WHERE uid=#{uid}")
...
public R<AdminUserListVO> getUsers(
    ...
    AdminUserListVO users = adminUserService.getUsers(page, size, keyword, status);
    return R.ok(users);
}
...
@PutMapping("/songs/{songId}/status")
public R<Void> updateSongStatus(
        @PathVariable String songId,
        @Valid @RequestBody UpdateStatusDTO dto,
        @RequestAttribute("adminId") Long adminId) {
