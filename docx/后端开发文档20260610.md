...
| 组件 | 技术                          | 版本         | 说明       |
| -- | --------------------------- | ---------- | -------- |
| 框架 | Spring Boot / NestJS        | 3.x / 10.x | 与主项目一致   |
| 前端 | 管理后台模板（如 Vue Element Admin） | -          | 独立于主项目前端 |
| 鉴权 | JWT + 管理员角色                 | -          | 独立 Token |
| 部署 | 与主项目同一容器                    | -          | 通过路径前缀区分 |
...
```json
{
  ...
  "data": {
    ...
    "songStats": {
      ...
      "onlineSongs": 48500,
      "vipSongs": 3200,
      "totalPlayCount": 128000000
    },
    "recentActions": [
      {"admin": "admin", "action": "禁用用户", "target": "U202606080001", "time": "2026-06-10T09:00:00Z"}
    ]
  }
}
```
